...
const EventSchema = new mongoose.Schema({
  ...
  department: { type: String, required: true, enum: ['CSE', 'IT', 'CSD', 'CSM', 'CSC', 'ECE', 'EEE', 'MECH', 'CIVIL', 'MBA', 'PACE'] },
  ...
  contact: { type: String },
  prize: { type: String },
  eventType: { type: String, enum: ['Technical', 'Non-Technical'], default: 'Technical' },
}, { timestamps: true });
...
app.post('/api/events', authMiddleware, (req, res, next) => {
  // Only use multer for multipart uploads; skip for JSON (image URL)
  ...
  try {
    ...
    // If image is a URL string passed in JSON, it stays in eventData.image already
...
app.get('/api/admin/analytics', authMiddleware, async (req, res) => {
  try {
    ...
    const depts = ['CSE', 'IT', 'CSD', 'CSM', 'CSC', 'ECE', 'EEE', 'MECH', 'CIVIL', 'MBA', 'PACE'];
